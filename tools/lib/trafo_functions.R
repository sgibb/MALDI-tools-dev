...
write.tab.uw <- function(dataframe, file)
{
...
{
  tr=solve(trans)
  writeAffine(tr,"data/aff")
  tr=readAffine("data/aff", target=imgMSI, source=imgHE)
  ...
  
{
  ...
  else
  {
    tr=trans
    writeAffine(tr,"data/aff")
    tr=readAffine("data/aff", target=imgHE, source=imgMSI)
